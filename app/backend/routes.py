    ...
    try:
        if request.method == 'POST':
            if 'file' not in request.files:
                ...
                return jsonify({'error': 'No file part'}), 400  # Returning JSON response with 400 Bad Request
            ...
            try:
                ...
                session['current_session_id'] = 'temp'
                ...
                return jsonify({'message': 'File uploaded successfully'}), 200  # Returning success
            except Exception as e:
                current_app.logger.error(f'{e}')
                return jsonify({'error': 'Error processing file'}), 500  # Internal server error
    except Exception as e:
...
@main.route('/upload_direct', methods=['GET','POST'])
def upload_direct():
    try:
        if request.method == 'POST':
            if 'directFile' not in request.files:
                current_app.logger.info('No file uploaded')
                flash('No file part')
                return redirect(request.url)

            file = request.files['directFile']
            ...
            try:
                file_content = file.read().decode('utf-8')
                print("CURRENT SESSION - " + session.get('current_session_id') )
                extract_product_data(file_content, session.get('current_session_id'))
                df = pd.read_csv(StringIO(file_content), on_bad_lines='warn')  
                write_db(df, session.get('current_session_id'))
                current_app.logger.info('File successfully processed direct data')
                return redirect(url_for('main.index'))
            except Exception as e:
                current_app.logger.error(f'{e}')
                flash(f'Error processing file')
...
def view_expenses():
    session_id = session.get('current_session_id')
    if not session_id:
        flash("Session ID not found. Please start a session.", "error")
        return redirect(url_for('main.index'))
